...
export const rateLimitMiddleware = (config?: {
  ...
  const options = {
    ...
    message: config?.message ? {
      ...
      message: config.message,
      error: 'RATE_LIMIT_EXCEEDED'
    } as APIResponse : defaultConfig.message,
...
export const aiRateLimit = rateLimitMiddleware({
  ...
  message: 'Too many AI requests, please wait before making more classification requests.'
});
...
export const blockchainRateLimit = rateLimitMiddleware({
  ...
  message: 'Too many blockchain requests, please wait before making more transactions.'
});
...
export const tierBasedRateLimit = (req: Request, res: Response, next: any) => {
  ...
  const tierRateLimit = rateLimitMiddleware({
    windowMs: 60 * 1000, // 1 minute
    max: maxRequests,
    message: `Too many requests for your tier (${user?.tier || 'Tier 1'}). Upgrade your tier for higher limits.`,
    keyGenerator: (req: Request) => `tier:${user?.tier || 'Tier 1'}:${req.userId || req.ip}`
  });
...
export const createRedisRateLimit = (redisClient?: any) => {
  if (!redisClient) {
    logger.warn('Redis client not provided, falling back to in-memory rate limiting');
